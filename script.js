...
(function initCircuitHero() {
  ...
  const lineMaterial = new THREE.LineBasicMaterial({ color: 0x3A5070, transparent: true, opacity: 0.35 });
  edges.forEach(([a, b]) => {
  ...
  nodes.forEach((pos, i) => {
    ...
    const material = new THREE.MeshBasicMaterial({ color: isCopper ? COPPER : CYAN, transparent: true, opacity: 0.85 });
  ...
  for (let i = 0; i < PULSE_COUNT; i++) {
    ...
    const material = new THREE.MeshBasicMaterial({ color: isCopper ? COPPER : CYAN });
